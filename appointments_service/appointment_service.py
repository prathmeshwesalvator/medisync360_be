...
def create_appointment(patient, doctor, slot, reason='') -> Appointment:
    ...
    # FIX: TimeSlot has no class constants — use string literals
    slot.status = 'booked'
    ...
    AppointmentReminder.objects.create(
        appointment=appt,
        remind_at=appt_dt - datetime.timedelta(days=1),
    )
    AppointmentReminder.objects.create(
        appointment=appt,
        remind_at=appt_dt - datetime.timedelta(hours=1),
    )
    return appt
...
def reschedule_appointment(appt: Appointment, new_slot: TimeSlot) -> Appointment:
    ...
    appt.save(update_fields=[
        ...
        'status', 'reschedule_count', 'updated_at',
    ])
    # Reset reminders
